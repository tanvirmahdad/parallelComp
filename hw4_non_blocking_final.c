...
cal_present&local_present&local_present&local_present
     ...
     // MPI_Sendrecv(&local_present[1][0],N+2,MPI_DOUBLE,prev,tag1,&local_present[local_row_end+1][0],N+2,MPI_DOUBLE,next,tag1,MPI_COMM_WORLD,MPI_STATUS_IGNORE);     
      ...
      MPI_Isend(&local_present[1][0],N+2,MPI_DOUBLE,prev,tag1,MPI_COMM_WORLD,&request[0]);
      MPI_Irecv(&local_present[0][0],N+2,MPI_DOUBLE,prev,tag2,MPI_COMM_WORLD,&request[1]);
      MPI_Isend(&local_present[local_row_end][0],N+2,MPI_DOUBLE,next,tag2,MPI_COMM_WORLD,&request[2]);
      MPI_Irecv(&local_present[local_row_end+1][0],N+2,MPI_DOUBLE,next,tag1,MPI_COMM_WORLD,&request[3]);
     ...
     // MPI_Barrier(MPI_COMM_WORLD);
      ...
      // printf("\n");
       if(comp_result_unified==comm_size){
          break;
       }
